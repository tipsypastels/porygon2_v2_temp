...
const experiment = (args) => {
    return array_1.random(experiments)(args);
};
const counter = async ({ intr, embed, channel }) => {
    let clicks = 0;
    embed
        .poryColor('ok')
        .poryThumb('speech')
    ...
    row.listen(channel, { time: time_1.Minutes(1) }).on('end', onEnd);
};
const potenuse = async ({ intr, embed, channel, author }) => {
    ...
    embed
        ...
        .setImage(assets_1.EXPERIMENT_ASSETS.get('potenuse').url);
    const onSelect = ({ intr }) => {
...
const lang = lang_1.createLang({
    ...
    potenuse: {
        ...
        result: '_Thank you for taking the experiment, {subject}! You said it made you feel **{emotion}**... interesting! I will make a note of that._',
        aborted: '_You took too long to answer, {subject}! Helping with important scientific research is too good for you, huh!_',
    },
});
